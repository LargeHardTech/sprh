...
灵感来源于二维图灵机模型，SPRH 提供了一个 **512×512 的字节网格**作为内存，一个**单字节寄存器**（变量），一个**字节栈**，以及丰富的指令集，包括算术、位运算、条件跳转、文件 I/O 等。它既可以作为**解释器**逐条运行，也可以通过内置的**编译器**转换为 C++ 代码，进而编译为本地机器码，获得接近原生的执行效率。

---

## ✨ 特性一览

- **二维内存网格**：512×512 个字节单元，通过 X/Y 指针访问。
